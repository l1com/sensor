...
void emio_init(void)
{
	...
	//将CAM1 sccb的SCLK和SDA都拉高
	XGpioPs_WritePin(&gpiops_inst, EMIO_SCL1_NUM, 1);
	XGpioPs_WritePin(&gpiops_inst, EMIO_SDA1_NUM, 1);

	//设置CAM2 sccb端口 为输出
	XGpioPs_SetDirectionPin(&gpiops_inst, EMIO_SCL2_NUM, 1);
	XGpioPs_SetDirectionPin(&gpiops_inst, EMIO_SDA2_NUM, 1);
	//使能CAM2 sccb端口 输出
	XGpioPs_SetOutputEnablePin(&gpiops_inst, EMIO_SCL2_NUM, 1);
	XGpioPs_SetOutputEnablePin(&gpiops_inst, EMIO_SDA2_NUM, 1);
	//将CAM2 sccb的SCLK和SDA都拉高
	XGpioPs_WritePin(&gpiops_inst, EMIO_SCL2_NUM, 1);
	XGpioPs_WritePin(&gpiops_inst, EMIO_SDA2_NUM, 1);
...
void sccb_start(u8 cam_ch)
{
    ...
    if(cam_ch == 0){
        ...
        emio_scl = EMIO_SCL2_NUM;
        emio_sda = EMIO_SDA2_NUM;
    }
